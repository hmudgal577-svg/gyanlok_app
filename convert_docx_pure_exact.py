...
def get_run_style(run, is_header=False):
    ...
    if font.color and font.color.rgb:
        ...
        # If color is D9E8F5 (very light blue), map to rich readable blue on white background
        if hex_color and hex_color.upper() == '#D9E8F5':
            hex_color = '#1E40AF' # Rich Navy Blue
        if hex_color:
            styles.append(f"color: {hex_color}")
    elif is_header:
        styles.append("color: #2563EB")
...
def docx_to_exact_html(filepath):
    ...
    # Extract Header (Pure exact DOCX colors & centered layout, no extra card box)
    for section in doc.sections:
        if section.header and section.header.paragraphs:
            header_paras = [paragraph_to_html(p, is_header=True) for p in section.header.paragraphs if p.text.strip()]
            if header_paras:
                html_parts.append(
                    f'<div class="doc-header" style="margin-bottom: 2rem; text-align: center; font-style: normal;">'
                    + "".join(header_paras) +
                    '</div>'
                )
            break
    ...
    # Extract Footer (Pure exact DOCX color & right layout)
    for section in doc.sections:
...
print("SUCCESS: CONVERTED DOCX WITH PURE EXACT HEADER COLORS & LAYOUT!")
